...
for name,item in pairs(Koaio.Weapons) do
  Weapons[Koaio.hash(name)] = item
end

for name,item in pairs(Koaio.AmmoTypes) do
  AmmoTypes[Koaio.hash(name)] = item
end
...
function GetAmmoItemFromHash(hash)
  for name,item in pairs(Koaio.Weapons) do
    if hash == Koaio.hash(item.name) then
      if item.ammo then
        return item.ammo
      else
        return nil
...
function RemoveUsedAmmo()  
  ...
  if ammoType and ammoType.item then
    ...
    if ammoDiff > 0 then
      KoaioClientUtils.Server('esx:discardInventoryItem', ammoType.item, ammoDiff)
    end
  end
...
Koaio.netHandleBasic('esx:modelChanged', function()
  RebuildLoadout()
end)

Koaio.netHandleBasic('playerSpawned', function()
  RebuildLoadout()
end)

Koaio.netHandleBasic('skinchanger:modelLoaded', function()
  RebuildLoadout()
end)

RegisterNetEvent('esx:addInventoryItem')
Koaio.netHandleBasic('esx:addInventoryItem', function(name, count)
...
Koaio.netHandleBasic('esx:removeInventoryItem', function(name, count)
  Citizen.Wait(1) -- Wait a tick to make sure ESX has updated PlayerData
  PlayerData = ESX.GetPlayerData()
  RebuildLoadout()
  if CurrentWeapon then

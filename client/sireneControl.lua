...
--Koaio.netRegisterAndHandle("lvc_TogIndicState_c", function(sender, newstate)
...
Koaio.netRegisterAndHandle("lvc_TogDfltSrnMuted_c", function(sender, toggle)
	local player_s = GetPlayerFromServerId(sender)
	local ped_s = GetPlayerPed(player_s)
	if DoesEntityExist(ped_s) and not IsEntityDead(ped_s) then
		if ped_s ~= GetPlayerPed(-1) then
...
Koaio.netRegisterAndHandle("lvc_SetLxSirenState_c", function(sender, newstate)
	local player_s = GetPlayerFromServerId(sender)
	local ped_s = GetPlayerPed(player_s)
	if DoesEntityExist(ped_s) and not IsEntityDead(ped_s) then
		if ped_s ~= GetPlayerPed(-1) then
...
Koaio.netRegisterAndHandle("lvc_TogPwrcallState_c", function(sender, toggle)
	local player_s = GetPlayerFromServerId(sender)
	local ped_s = GetPlayerPed(player_s)
	if DoesEntityExist(ped_s) and not IsEntityDead(ped_s) then
		if ped_s ~= GetPlayerPed(-1) then
...
Koaio.netRegisterAndHandle("lvc_SetAirManuState_c", function(sender, newstate)
	local player_s = GetPlayerFromServerId(sender)
	local ped_s = GetPlayerPed(player_s)
	if DoesEntityExist(ped_s) and not IsEntityDead(ped_s) then
		if ped_s ~= GetPlayerPed(-1) then
				...
				if GetPedInVehicleSeat(veh, -1) == playerped then
					...
					if GetVehicleClass(veh) ~= 14 and GetVehicleClass(veh) ~= 15 and GetVehicleClass(veh) ~= 16 and GetVehicleClass(veh) ~= 21 then
						...
						if count_bcast_timer > delay_bcast_timer then
							...
							--- IS EMERG VEHICLE ---
							if GetVehicleClass(veh) == 18 then
								KoaioClientUtils.toServer("lvc_TogDfltSrnMuted_s", dsrn_mute)
								KoaioClientUtils.toServer("lvc_SetLxSirenState_s", state_lxsiren[veh])
								KoaioClientUtils.toServer("lvc_TogPwrcallState_s", state_pwrcall[veh])
								KoaioClientUtils.toServer("lvc_SetAirManuState_s", state_airmanu[veh])
							end
							--- IS ANY OTHER VEHICLE ---
							--KoaioClientUtils.toServer("lvc_TogIndicState_s", state_indic[veh])
						else

...
local callTakeHostage = function()
	...
	if not canTakeHostage then 
		alert(_U("otage_info_player_arme"))
	end
	...
	if not takeHostage.InProgress and canTakeHostage then			
		...
		if closestPlayer then
			...
			if targetSrc ~= -1 then
				...
				ensureAnim(takeHostage.agressor.animDict)
				takeHostage.type = "agressor"
			else
				alert("~r~".._u("not_player"))
			end
		else
			alert("~r~".._u("not_player"))
		end
...
AddEventHandler("TakeHostage:syncTarget", function(target)
	...
	takeHostage.InProgress = true
	ensureAnim(takeHostage.hostage.animDict)
...
AddEventHandler("TakeHostage:killHostage", function()
	...
	DetachEntity(PlayerPedId(), true, false)
	ensureAnim("anim@gangops@hostage@")
...
Citizen.CreateThread(function()
	while true do 
		if takeHostage.type == "agressor" then
			...
			drawNativeText(_U("otage_info_kill_player"),1000)
